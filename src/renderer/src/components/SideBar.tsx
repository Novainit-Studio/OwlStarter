...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { id: 'status', icon: BarChart2, label: '伺服器狀態' },
  ...
  return (
    <div className={`relative h-full bg-[#1f1f1f] border-r border-[#2b2b2b] transition-all duration-300 ease-in-out ${collapsed ? 'w-16' : 'w-60'} flex flex-col`}>
      {/* 頂部標題 */}
      <div className="flex items-center p-4 border-b border-[#2b2b2b]">
        <img src="/src/assets/logo.png" alt="Logo" className='w-14 pr-1' />
        {!collapsed && (
          <h1 className="font-bold text-white truncate">OwlStarter</h1>
        )}
      </div>
      ...
      {/* 底部登出按鈕 */}
      <div className={`p-4 border-t border-[#2b2b2b] flex items-center ${collapsed ? 'justify-center' : ''} cursor-pointer hover:bg-[#2b2b2b] hover:bg-opacity-50 transition-colors duration-200`}>
        <LogOut 
          size={20} 
          className={`text-gray-400 ${collapsed ? '' : 'mr-3'}`} 
        />
        {!collapsed && (
          <span className="text-sm text-gray-400">登出</span>
        )}
      </div>
    </div>
  )
